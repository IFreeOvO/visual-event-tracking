...
export function getRegisteredViewExtensions() {
    const res = registeredViewExtensions.filter(view => {
        const isValidatePanel =(view.viewRegistration.location === 'panel' && !['console','security','cssoverview','network','heap-profiler','lighthouse','resources','sources','medias','timeline','layers','chrome-recorder'].includes(view.viewRegistration.id))
        return isValidatePanel
    })
    return res
